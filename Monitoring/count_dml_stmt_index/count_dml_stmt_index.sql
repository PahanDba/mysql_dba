...
DROP PROCEDURE if exists assistant_dba.`count_dml_stmt_index`;
DELIMITER $$
CREATE PROCEDURE assistant_dba.`count_dml_stmt_index`()
begin
...
#call `assistant_dba`.`count_dml_stmt_index` ();
...
if @var_first_number_server_version<8 then 
	set @check_version=5;
    #I'll create the table which called t_server_idsevernamecount_dml_stmt_index. For example, the global variables has values server_id=3043 and hostname=mysql3043.
    #The table will be named t_3043_mysql3043_count_dml_stmt_index.
    #I remove from the hostname the characters that do not satisfy the condition [a-z] or [A-Z] or [0-9] or _.
    set @tblname_count_dml_stmt_index=concat('t_',@var_server_id,'_',(select assistant_dba.`regexp_replace`(@var_server_name ,'[^a-zA-Z0-9_]','')),'_count_dml_stmt_index');
...
if @var_first_number_server_version>=8 then 
    ...
    #The table will be named t_3043_mysql3043_count_dml_stmt_index.
    #I remove from the hostname the characters that do not satisfy the condition [a-z] or [A-Z] or [0-9] or _.
    set @tblname_count_dml_stmt_index=concat('t_',@var_server_id,'_',(select regexp_replace(@var_server_name ,'[^a-zA-Z0-9_]','')),'_count_dml_stmt_index');
end if;  
	set @sql_create_tbl :=concat(' CREATE TABLE if not exists \`assistant_dba\`.',@tblname_count_dml_stmt_index,' 
									(\`id\` INT UNSIGNED NOT NULL AUTO_INCREMENT, \`object_type\` VARCHAR(64) NOT NULL, 
                                    \`object_schema\`  VARCHAR(64) NOT NULL, \`object_name\`  VARCHAR(64) NOT NULL, \`index_name\`  VARCHAR(64) NOT NULL, 
	...
	set @sql_insert :=concat(' insert into \`assistant_dba\`.',@tblname_count_dml_stmt_index,' 
								(\`object_type\` , \`object_schema\`, \`object_name\` , \`index_name\`, \`count_star\`, \`count_read\` , 
                                \`count_write\`, \`count_fetch\` , \`count_insert\`, \`count_update\`, \`count_delete\`, 
                                \`date_collect\`,\`date_start\`)  
								SELECT object_type, object_schema, object_name, COALESCE(index_name, ''NULL'') as index_name, count_star, count_read, count_write, 
...
DROP EVENT if exists assistant_dba.`count_dml_stmt_index`;
DELIMITER $$
CREATE EVENT assistant_dba.`count_dml_stmt_index` ON SCHEDULE EVERY 30 SECOND STARTS '2024-12-14 00:00:00' ON COMPLETION NOT PRESERVE ENABLE COMMENT 'Collecting information how many DML operations were in each ind.' 
DO BEGIN
   call assistant_dba.count_dml_stmt_index(); 
 END$$
 DELIMITER ;
...
DELIMITER ;
DROP PROCEDURE if exists assistant_dba.`count_dml_stmt_index_analyze`;
DELIMITER $$
CREATE PROCEDURE assistant_dba.`count_dml_stmt_index_analyze`(in in_table_name_store varchar(64), in_schema_name_my varchar(64), in_table_name_my varchar(64), date_collect_start_input datetime, date_collect_finish_input datetime )
begin
/*
...
of statements from the table performance_schema.table_io_waits_summary_by_index_usage. This table is always there in the database called 'assistant_dba'.
...
#The MySQL server didn't work this day.
#call assistant_dba.count_dml_stmt_index_analyze('t_3043_mysql3043_count_dml_stmt_index', 'tempor_t4','users1_2m' ,'2025-10-25 14:15:30','2025-10-25 15:15:30');
#the starting time equals the finishing time
#call assistant_dba.count_dml_stmt_index_analyze('t_3043_mysql3043_count_dml_stmt_index', 'tempor_t4','users1_2m' ,'2025-10-26 14:15:30','2025-10-26 14:15:30');
#The finishing date and time of the interval is less than the starting date and time of the interval
#call assistant_dba.count_dml_stmt_index_analyze('t_3043_mysql3043_count_dml_stmt_index', 'tempor_t4','users1_2m' ,'2025-10-26 18:38:30','2025-10-26 17:38:30');
...
#call assistant_dba.count_dml_stmt_index_analyze('t_3043_mysql3043_count_dml_stmt_index', 'tempor_t4','users1_2m' ,'2025-10-23 10:00:30','2025-10-26 14:52:30');
...
#call assistant_dba.count_dml_stmt_index_analyze('t_3043_mysql3043_count_dml_stmt_index', 'tempor_t4','users1_2m' ,'2025-10-26 14:00:30','2025-10-26 15:12:30');
...
#call assistant_dba.count_dml_stmt_index_analyze('t_3043_mysql3043_count_dml_stmt_index', 'tempor_t4','users1_2m' ,'2025-10-26 15:30:30','2025-10-26 18:29:30');    
...
set @table_name_store_tmp:=concat(@table_name_store,'_tmp_index');
...
if @check_date_collect_start_input is null or @check_date_collect_start_input='' or @check_date_collect_start_input=' '
    ...
    if @date_collect_start_input_more is null or @date_collect_start_input_more='' or @date_collect_start_input_more=' '
    then 
     set @date_collect_start_input_more=@date_collect_start_input_less;
    end if;
...
if time_to_sec(timediff(@date_start_beg_srv,@date_start_fin_srv))=0 then
	...
	set @create_temptbl_good:=concat('create  table assistant_dba.',@table_name_store_tmp,' (id int(10) NOT NULL AUTO_INCREMENT, object_type varchar(64) NOT NULL, object_schema varchar(64) NOT NULL, object_name varchar(64) NOT NULL, index_name varchar(64) NOT NULL, count_star bigint(20) NOT NULL, count_read bigint(20) NOT NULL,  count_write bigint(20) NOT NULL, count_fetch bigint(20) NOT NULL, count_insert bigint(20) NOT NULL, count_update bigint(20) NOT NULL, count_delete bigint(20) NOT NULL, date_collect datetime NOT NULL, date_start datetime NOT NULL, comment_my varchar(500) NULL, PRIMARY KEY (id)); ');
    PREPARE st8_3 FROM @create_temptbl_good;
    EXECUTE st8_3;
    DEALLOCATE PREPARE st8_3;

	set @date_1_stmt:=concat('insert into assistant_dba.',@table_name_store_tmp,' (object_type, object_schema, object_name, index_name, count_star, count_read,
			count_write, count_fetch, count_insert, count_update, count_delete, date_collect, date_start, comment_my)
			select t1.object_type, t1.object_schema, t1.object_name, t1.index_name, t1.count_star, t1.count_read, t1.count_write, 
			t1.count_fetch, t1.count_insert,
			t1.count_update, t1.count_delete, t1.date_collect, t1.date_start , 
			''These are the values of the DML operations on the starting date and time of the interval and the finishing date and time of the interval.'' 
			FROM assistant_dba.',@table_name_store,' t1
			join 
			(
			select object_type, object_schema, object_name, index_name, 
					min(date_collect) as min_date_collect, max(date_collect) as max_date_collect, date_start  
					...
					group by object_type, object_schema, object_name, index_name, date_start
			) t2
			on t2.object_schema=t1.object_schema and t2.object_name=t1.object_name and t2.index_name=t1.index_name
			where t1.date_collect=t2.min_date_collect or t1.date_collect=t2.max_date_collect
			order by t1.index_name, t1.date_collect;');
    ...
    set @date_2_stmt:=concat('select ''These are the values of the DML operations on the starting date and time of the interval and the finishing date and time of the interval.''  as description,
							object_schema, object_name, index_name, count_star, count_read, count_write, count_fetch, count_insert, count_update, count_delete, '' '' as ''delta_sec'',
                            date_collect, date_start
                            ...
                            t1.object_schema, t1.object_name, t1.index_name,
							...
							on  t2.index_name=t1.index_name  and t1.date_collect<t2.date_collect 
                            union all
                            ...
                            FROM assistant_dba.',@table_name_store_tmp,' t1 
							join assistant_dba.',@table_name_store_tmp,' t2
							on t2.date_start=t1.date_start and t2.date_collect>t1.date_collect and t2.index_name=t1.index_name
                            order by 14, 13	;');
...
if time_to_sec(timediff(@date_start_beg_srv,@date_start_fin_srv))<>0 then
	...
	set @create_temptbl_good:=concat('create  table assistant_dba.',@table_name_store_tmp,' (id int(10) NOT NULL AUTO_INCREMENT, 
									object_type varchar(64) NOT NULL, object_schema varchar(64) NOT NULL, object_name varchar(64) NOT NULL, index_name varchar(64) NOT NULL, 
    ...
    set @date_20_stmt:=concat('insert into assistant_dba.',@table_name_store_tmp,' (object_type, object_schema, object_name, index_name, count_star,
								count_read, count_write, count_fetch, count_insert, count_update, count_delete, date_collect, date_start)
								select t1.object_type, t1.object_schema, t1.object_name, t1.index_name, t1.count_star, t1.count_read,
								t1.count_write, t1.count_fetch, t1.count_insert, t1.count_update, t1.count_delete, t1.date_collect, t1.date_start
								from assistant_dba.',@table_name_store,' t1
								join (select index_name, date_start, min(date_collect) as min_date_collect, max(date_collect) as max_date_collect 
									  FROM assistant_dba.',@table_name_store,'
									  where date_start in (
										...
										object_schema=''',@schema_name_my,''' and object_name=''',@table_name_my,'''
										group by index_name, date_start 
    ...
    set @date_21_stmt:=concat('select ''These are the values of the DML operations on the starting date and time of the interval and the finishing date and time of the interval.''  as description,
                            ...
                            FROM assistant_dba.',@table_name_store_tmp,' t1 
							join assistant_dba.',@table_name_store_tmp,' t2
							on t2.index_name=t1.index_name and t2.date_start=t1.date_start and t2.date_collect>t1.date_collect
                            union all
							select ''This is the average value of the DML operations per second between the starting date and time of the interval and the finishing date and time of the interval.'' as description, 
                            t1.object_schema, t1.object_name, t1.index_name,
                            (t2.count_star-t1.count_star)/time_to_sec(timediff (t2.date_collect,t1.date_collect)) as delta_count_star,
