...
#sudo sed -i -e 's/\r$//' /etc/mysql/dba_scripts/users/users2_mysql.sh
#sudo chown root  /etc/mysql/dba_scripts/users/users2_mysql.sh
#sudo chmod 755  /etc/mysql/dba_scripts/users/users2_mysql.sh
...
path_config_mysql_backup="/etc/mysql/dba_conf/users/"
tools_run="/usr/bin/mysql"
...
query_get_user=("select  "$in_server_id" as server_id, "\"$in_server_name\"" as server_name,user, host, account_locked,
password_expired,
'E' AS is_role 
from mysql.user;
")
...
$tools_run   --defaults-extra-file=$path_config_mysql_users$in_server_name.cnf  -e "$query_get_user" > $path_out_users_list 2>$path_file_users_error #$path_out_db_list
echo "query_tbl_users: $query_tbl_users;"   >>  $path_file_users_all
...
echo "Import users from on $server_name $query_load_users;"   >>  $path_file_users_all
...
$tools_run  --defaults-extra-file=$path_config_mysql_users$in_server_name.cnf -e "call mysql.dba_user_priv_mysql() ;" > $path_out_privileges_list 2>$path_file_users_error #&1 	
...
echo "insert into history_mysql_privileges from $in_server_name $(date +"%Y%m%d %H:%M:%S")"   >>  $path_file_users_all
$tools_run  --defaults-extra-file=$path_config_mysql_users$conn_server.cnf -e "call assistant_dba.save_track_user_permissions_mysql('$conn_db_main_temp','$tbl_srv_priv',$in_server_id) ;" >> $path_file_users_all 2>$path_file_users_error #&1 
