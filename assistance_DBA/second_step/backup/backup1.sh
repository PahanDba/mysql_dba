...
run_script_mysql="backup2_xtrabackup.sh"
...
results_check_count=( $($tools_run  --defaults-extra-file=$path_config_mysql_backup$conn_server.cnf -N -D $conn_db -e "call assistant_dba.run_task_mysql(0);"  2>$path_file_main_error_log_backup_server) )
...
if (( $results_check_count > 0)); then
	...
	while ifs=$'\t' read task_id bt_id backup_type_description creat_time wait_task_id enable file_log server_id server_name version ip backup_task_name path_backup path_last_backup path_last_full_backup path_last_incr_backup server_for_backup user_for_backup process_last_full_backup prepare_last_full_backup process_last_incr_backup prepare_last_incr_backup ;do
		...
		if [[ "$version1" == *"mariadb"* ]]; then
			...
			source $script_main_dir/$run_script_mysql $task_id $bt_id $backup_type_description $creat_time $wait_task_id $enable $file_log $server_id $server_name $version $ip $backup_task_name $path_backup $path_last_backup $path_last_full_backup $path_last_incr_backup $server_for_backup $user_for_backup $process_last_full_backup $prepare_last_full_backup $process_last_incr_backup $prepare_last_incr_backup & 2>$path_file_main_error_log_backup_server 		
		fi
